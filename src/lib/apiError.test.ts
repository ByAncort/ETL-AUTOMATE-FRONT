...
describe('mapApiError', () => {
  it('API sin respuesta (servidor caido) -> mensaje de conexión', () => {
    const err = { request: {}, response: undefined } as unknown as AxiosError<{ message?: string }>;
  ...
  it('timeout (ECONNABORTED) -> mensaje de demora', () => {
    const err = { code: 'ECONNABORTED', response: undefined } as unknown as AxiosError<{ message?: string }>;
  ...
  it('error 500 -> mensaje de error del servidor (no expone detalle)', () => {
    const err = { response: { status: 500, data: { message: 'NullPointer en H2' } } } as unknown as AxiosError<{ message?: string }>;
    expect(mapApiError(err)).toBe('Error del servidor. Intenta más tarde.');
  });

  it('error 404 -> usa mensaje del servidor o fallback', () => {
    expect(mapApiError({ response: { status: 404, data: {} } } as unknown as AxiosError<{ message?: string }>)).toBe('Recurso no encontrado.');
    expect(mapApiError({ response: { status: 404, data: { message: 'API no existe' } } } as unknown as AxiosError<{ message?: string }>))
      .toBe('API no existe');
  });

  it('401/403 -> mensaje de autorización', () => {
    expect(mapApiError({ response: { status: 401, data: {} } } as unknown as AxiosError<{ message?: string }>))
      .toBe('No tienes autorización para esta acción.');
    expect(mapApiError({ response: { status: 403, data: {} } } as unknown as AxiosError<{ message?: string }>))
      .toBe('No tienes autorización para esta acción.');
  });

  it('400 con mensaje del backend -> propaga el mensaje', () => {
    const err = { response: { status: 400, data: { message: 'url is required' } } } as unknown as AxiosError<{ message?: string }>;
